...
def quantize_models_weights(model, bits):
    q_model = copy.deepcopy(model)

    for name, param in q_model.named_parameters():
        if param.requires_grad and param.dim() > 1:  # skip biases
            w = param.data
            w_min = w.min()
            w_max = w.max()
            param.data = quantize_gen(w_min, w_max, bits, w)
...
for mode in modes:
    ...
    for bits in bit_configs:
        ...
        q_model = model if bits is None else quantize_models_weights(model, bits)

        handle = q_model.transformer.h[2].register_forward_hook(...)
        outputs = q_model(batch, labels=batch)            
        wandb.init(
...
for mode in modes:
    ...
    save_dir = "assign_2_bonus"
    os.makedirs(save_dir, exist_ok=True)

    plt.tight_layout()
    plt.savefig(f"{save_dir}/umap_fixed_{mode}_{m_bottleneck}_wt_quant.png", dpi=300)
    print(f"Saved: {save_dir}/umap_fixed_{mode}.png")